...
class TaskSolver:
    ...
    def set_box_and_shelf_positions_1(self, box_pose: Dict[str, float], shelf_pose: Dict[str, float]) -> bool:
        ...
        try:
            ...
            box_pose_msg.position = Point(
                x=box_pose.get('x', 3.15),# 获取x坐标，默认3.15
            ...
            if not response.success:
                ...
                return False
            rospy.loginfo(f"Setting shelf position: {shelf_pose_msg}")
    ...
    def next_action(self, obs: dict) -> dict:
        ...
        if self.i == 0:
            self.pos_init = body_state["world_position"]
            self.i +=1
        ...
        #         else:
        #             self.get_cmd(0, 0, 0, -0.6)
        #     elif np.sqrt(distance) < 0.1:  # 到达阈值
        #         rpy = self.quat2euler(quat, "sxyz") 
        #         print(f"yaw: {rpy[2]}")
        #         if rpy[2] > 0.1:
        #             self.get_cmd(0, 0, 0, -0.3)
        ...
        self.started = True
        if self.started and self.j == 0:
            self.launch_grab_box() 
            self.set_box_and_shelf_positions(self.pos_init)
