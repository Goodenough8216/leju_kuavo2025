...
class TaskSolver:
    ...
    def control_arm(self, target_pos_id):
        ...
        if target_pos_id == 0:
            ...
            # 设置目标位置
            target_poses = np.array([-10,0,-20,-80,40,20,0,-10,0,20,-80,-40,-20,0])
        elif target_pos_id == 6:
            # 设置目标位置
            target_poses = np.array([-40,0,-20,-80,40,20,0,-40,0,20,-80,-40,-20,0])
        elif target_pos_id == 7:
            # 设置目标位置
            target_poses = np.array([0,0,90,0,0,0,0,0,0,-90,0,0,0,0])
        elif target_pos_id == 8:
            # 设置目标位置
            target_poses = np.array([20,0,-20,-110,40,20,0,20,0,20,-110,-40,-20,0])
        elif target_pos_id == 9:
            # 设置目标位置
            target_poses = np.array([0,0,-20,-120,40,20,30,0,0,20,-120,-40,-20,30])
        elif target_pos_id == 10:
            # 设置目标位置
            target_poses = np.array([-20,0,-20,-130,20,20,60,-20,0,20,-130,-20,-20,60])
        elif target_pos_id == 11:
            # 设置目标位置
            target_poses = np.array([-40,0,-20,-100,40,20,30,-40,0,20,-100,-40,-20,30])
        elif target_pos_id == 12:
            # 设置目标位置
            target_poses = np.array([-50,0,-20,-70,40,20,10,-50,0,20,-70,-40,-20,10])
        elif target_pos_id == 13:
            # 设置目标位置
            target_poses = np.array([-70,0,-20,-40,40,20,0,-70,0,20,-40,-40,-20,0])
        elif target_pos_id == 14:
            # 设置目标位置
            target_poses = np.array([-60,0,20,-60,40,0,0,-60,0,-20,-60,-40,0,0])
        elif target_pos_id == 15:
            # 设置目标位置
            target_poses = np.array([-50,0,0,-80,40,0,0,-50,0,0,-80,-40,0,0]) 
        elif target_pos_id == 16:
            # 设置目标位置
            target_poses = np.array([-35,0,0,-60,40,0,0,-35,0,0,-60,-40,0,0]) 
        elif target_pos_id == 17:
            # 设置目标位置
            target_poses = np.array([-20,0,0,-40,40,0,0,-20,0,0,-40,-40,0,0])  
    ...
    def detect_marker(self, obs: dict) -> list:
        ...
        try:
            ...
            with np.errstate(all='ignore'):  # 忽略numpy警告
                ...
                # 像素面积阈值
                COLOR_THRESHOLDS = {'red': 500, 'yellow': 300, 'blue': 400}
                ...
                for color in colors:
                    ...
                    if color == 'red':
                        ...
                        del mask1;del mask2  # 显式释放
                    else:
                        lower, upper = COLOR_RANGES[color]
                        mask = cv2.inRange(hsv, np.array(lower), np.array(upper))
                    
                    # 轻量级形态学操作
                    cv2.morphologyEx(mask, cv2.MORPH_OPEN, kernel, dst=mask)  # 原地操作
                    
                    # 像素计数检测
                    if cv2.countNonZero(mask) >= COLOR_THRESHOLDS[color]:
                        detected_colors.append(color)
                    
                    del mask  # 显式释放
                    
        except Exception as e:
            rospy.logerr(f"颜色检测错误: {str(e)}")
        finally:
            # 确保所有资源释放
            if 'hsv' in locals(): del hsv
            if 'mask' in locals(): del mask
            if 'mask1' in locals(): del mask1
            if 'mask2' in locals(): del mask2
            ...
            #     self.get_cmd(0, 0, 0.1, 0)
                ...
                if abs(diff_rot) > 0.1:
                    if abs(diff_rot) < 0.20 :
                        if(diff_rot > 0):
                            self.get_cmd(cmdx, cmdy, 0.1, diff_rot)
                            # print("turn right")
                        elif(diff_rot < 0):
                            self.get_cmd(cmdx, cmdy, 0.1, diff_rot)
                            # print("turn left")
