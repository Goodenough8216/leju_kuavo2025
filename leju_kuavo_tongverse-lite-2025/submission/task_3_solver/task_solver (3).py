...
class TaskSolver:
    ...
    def process_task_info(self, robot_init_pos):
        """处理任务信息，提取箱子和货架位置
        ...
        # 提取货架位置
        shelves_info = self.task_params['shelve_world_position']
        ...
        for key in boxes_info:
            ...
            boxes_pos_temp = boxes_info[key] - robot_offset
            boxes_info[key][0] = ( boxes_pos_temp[0] - 0.2 ) * np.cos(self.init_yaw) + boxes_pos_temp[1] * np.sin(self.init_yaw)
            boxes_info[key][1] = - ( boxes_pos_temp[0] - 0.2 ) * np.sin(self.init_yaw) + boxes_pos_temp[1] * np.cos(self.init_yaw)
            boxes_info[key][2] = 0.8
            # print(f"key：{key}, boxes_info[key]：{boxes_info[key]}")    
        for key in shelves_info:
            shelves_pos_temp = shelves_info[key] - robot_offset
            shelves_info[key][0] = shelves_pos_temp[0] * np.cos(self.init_yaw) + shelves_pos_temp[1] * np.sin(self.init_yaw)
            shelves_info[key][1] = -shelves_pos_temp[0] * np.sin(self.init_yaw) + shelves_pos_temp[1] * np.cos(self.init_yaw)
            shelves_info[key][2] = 1.2
        return boxes_info, shelves_info
    

    def set_box_and_shelf_positions_1(self, box_pose: Dict[str, float], shelf_pose: Dict[str, float]) -> bool:
        """设置箱子和架子的位置（包含朝向旋转）
        ...
        try:
            ...
            response = set_tag_pose(tag_id=3, pose=box_pose_msg)
    ...
    def set_box_and_shelf_positions(self, robot_pos, color):
        # 处理任务信息
        boxes_info, shelves_info = self.process_task_info(robot_pos)
        
        # 设置第一个箱子(红箱子)和对应货架的位置
        # select_color = 'red' # red blue yellow
        select_color = color
        box_pose = {
            'x': float(boxes_info[select_color + '_box'][0]),
        ...
        # # 设置位置
        success = self.set_box_and_shelf_positions_1(box_pose, shelf_pose)
    ...
    def next_action(self, obs: dict) -> dict:
        ...
        #     #     self.get_cmd(0, 0, 0, 0.3)
            ...
            if self.j == 0:
                ...
                # self.init_yaw = self.quat2euler(quat, "sxyz")[2]
                self.set_box_and_shelf_positions(self.init_pos, self.target_color[0])
            elif self.j == 60000:
                # self.init_yaw = self.quat2euler(quat, "sxyz")[2]
                self.set_box_and_shelf_positions(self.init_pos, self.target_color[2])
            if self.j == 0 or self.j == 30000 or self.j == 60000:
