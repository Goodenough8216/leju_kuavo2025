...
class TaskSolver:
    ...
    def process_obs(self, obs: Dict[str, Any], color ='red' , republish = False) :
        """处理观测数据并发布传感器数据
        ...
        Returns:
            marker:{
                'color': 检测到的颜色,
                'pixel_pos': (x,y)像素坐标,
                'camera_pos': (x,y,z)相机坐标系坐标,
                'robot_pos': (x,y,z)机器人坐标系坐标
            } 或 None(未检测到)
        """
        ...
        if "camera" in obs:
            ...
            if "world_pose" in camera_data:
                try:
                    print("处理相机位姿数据")
                    cam_pos, cam_quat = camera_data["world_pose"]
                    marker =  self.detect_marker(camera_data, color, former_intrinsics)
                    if marker:
                            world_pos = self.detected_marker['world_pos']
                            print("标记世界坐标:", world_pos)
                            self.publish_marker(world_pos)
                except Exception as e:
                    rospy.logerr(f"Failed to process camera world pose: {str(e)}")

        # if "extras" in obs:
        #     extras = obs["extras"]
        #     print(f"extras: {extras}")
        #     # 一旦出现instrinsics字段，直接退出程序
        #     if "intrinsics" in extras:
        #         rospy.loginfo("Camera intrinsics found, exiting...")
        #         rospy.signal_shutdown("Camera intrinsics found, exiting...")
        #         return

            # if "intrinsics" in camera_data:
            #     try:
            #         intrinsics = camera_data["intrinsics"]
            #         rospy.loginfo(f"Camera intrinsics: {intrinsics}")
            #         # 检测红色标记
            #         red_marker = self.detect_marker(camera_data, 'red', intrinsics)
            #         if red_marker:
            #             rospy.loginfo(f"Detected red marker: {red_marker}")


            #     except Exception as e:
            #         rospy.logerr(f"Failed to detect markers: {str(e)}")
        #--------------------------------------
        # 处理IMU数据
        if "imu_data" in obs:
    ...
    def detect_marker(self, camera_data, color, intrinsics):
        """
        检测指定颜色的标记并返回其在机器人坐标系中的位置
        
        参数:
            camera_data: 包含以下键的字典
                'rgb': (H,W,3)的RGB图像数组
                'depth': (H,W)的深度图像数组(单位:米)
                'world_pose': (position, quaternion)相机在世界坐标系中的位姿
            color: 要检测的颜色('red','blue','yellow')
        ...
        if len(valid_depths) == 0:
            print(f"No valid depth values found for color {color} at pixel ({cx}, {cy})")
            return None
        ...
        cx_cam, cy_cam = intrinsics['cx'], intrinsics['cy']
        ...
        # 旋转和平移变换
        point_world = camera_pos + rot.apply(point_camera)
        
        return {
            'color': color,
            'pixel_pos': (cx, cy),
            'camera_pos': (x, y, z),
            'world_pos': point_world.tolist()
        }
    def publish_marker(self, world_pos, marker_id=0):
        """
        发布 Marker 到 RViz 中显示
        Args:
            world_pos: (x, y, z) 世界坐标系中的位置
            marker_id: Marker 的 ID，用于区分多个标记
        """
        marker = Marker()
        ...
        marker.id = marker_id
        if marker_id == 0 : marker.type = Marker.SPHERE  # 使用球体表示
        else: marker.type = Marker.CUBE  # 使用立方体表示
    ...
    def control_arm(self, target_pos_id):
        ...
        if target_pos_id == 0:
            ...
            # 设置目标位置
            target_poses = np.array([-20,0,-30,-150,40,20,60,-20,0,30,-140,-40,-20,60])
        elif target_pos_id == 10:
            # 设置目标位置
            target_poses = np.array([-50,0,-30,-90,40,20,30,-50,0,30,-80,-40,-20,30])
        elif target_pos_id == 11:
            # 设置目标位置
            target_poses = np.array([-80,0,-30,-50,40,20,10,-80,0,30,-50,-40,-20,10])
        elif target_pos_id == 12:
            # 设置目标位置
            target_poses = np.array([-90,0,-30,-20,40,20,0,-90,0,30,-20,-40,-20,0])
        elif target_pos_id == 13:
            # 设置目标位置
            target_poses = np.array([-100,0,-30,0,40,20,-20,-100,0,30,0,-40,-20,-20])
        elif target_pos_id == 14:
            # 设置目标位置
            target_poses = np.array([-100,20,10,-20,40,0,0,-100,-20,-10,-20,-40,0,0]) 
        elif target_pos_id == 15:
            # 设置目标位置
            target_poses = np.array([-80,20,0,-60,40,0,0,-80,0,-20,-60,-40,0,0])
    ...
    def next_action(self, obs: dict) -> dict:
        ...
        if self.step_count % 4 == 0:
            if self.process_obs(obs,color = self.target_color[self.step_count//4]) != None:
                self.detected_marker = self.process_obs(obs,color = self.target_color[self.step_count//4])
        print(f"detected_marker: {self.detected_marker}")
        print("original self.boxes_info red", self.boxes_info['red_box'])
        self.publish_marker(self.boxes_info['red_box'], marker_id=1)
