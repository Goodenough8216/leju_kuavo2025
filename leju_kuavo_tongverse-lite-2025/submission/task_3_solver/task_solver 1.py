...
class TaskSolver:
    ...
    def process_task_info(self, robot_init_pos):
        ...
        for key in boxes_info:
            ...
            boxes_info[key][0] = boxes_pos_temp[0] * np.cos(self.init_yaw) + boxes_pos_temp[1] * np.sin(self.init_yaw)
            boxes_info[key][1] = - boxes_pos_temp[0] * np.sin(self.init_yaw) + boxes_pos_temp[1] * np.cos(self.init_yaw)
    ...
    def set_box_and_shelf_positions(self, robot_pos):
        # 处理任务信息
        ...
        box_pose = {
            'x': float(boxes_info[select_color + '_box'][0]) - 0.15,
    ...
    def next_action(self, obs: dict) -> dict:
        ...
        elif self.started and self.j == 0:
             
            self.launch_grab_box()
            self.set_box_and_shelf_positions(pos)
